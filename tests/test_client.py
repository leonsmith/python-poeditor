...
class TestClient(unittest.TestCase):
    ...
    def test_authentication(self):
        ...
        self.assertEqual(
            context.exception.message, "Status 'fail', code 4011: Invalid API Token"
        )
    ...
    def test_scenario(self):
        ...
        details = client.delete_terms(
            project_id=self.new_project_id,
            data=[{"term": "There is 1 student in the band", "context": ""}],
        )
            ...
            data=[
                {
                    "term": "Welcome to my new website",
                    "context": "",
                    "translation": {"content": "Bienvenue sur mon site", "fuzzy": 1},
                }
            ],
        )
        self.assertEqual(details["parsed"], 1)
        ...
        with open(self.file_path, "w") as sources:
            for line in lines:
                sources.write(
                    re.sub(
                        r'^msgstr "Bienvenue sur mon site"', 'msgstr "Bienvenue!"', line
                    )
                )
        ...
        expected_dict = {
            "translations": {"added": 0, "parsed": 1, "updated": 1},
            "terms": {"added": 0, "deleted": 0, "parsed": 2},
        }
        ...
        result = client.add_administrator(
            project_id=self.new_project_id, name="John", email="john@example.org"
        )
        self.assertTrue(result)
    ...
    def tearDown(self):
        ...
        if hasattr(self, "file_path"):
            try:
                logger.info(
                    "Removing temp test file file_path={}. ".format(self.file_path)
                )
